...
export type RequestParamsValidator<Config extends RequestParamsConfig> = Config["validator"];
export type RequestParamsValidate<Config extends RequestParamsConfig> = Config["validate"];
...
export type RequestParamsConfigInValidator<Config extends RequestParamsConfigIn> = StandardSchemaV1<
  StandardSchemaV1.InferInput<Config["validator"]>,
  StandardSchemaV1.InferOutput<Config["validator"]>
>;
export type RequestParamsConfigInValidate<Config extends RequestParamsConfigIn> =
  "validate" extends keyof Config ? Config["validate"] : undefined;

const configInValidator = <Config extends RequestParamsConfigIn>(config: Config) =>
  config.validator as RequestParamsConfigInValidator<Config>;
const configInValidate = <Config extends RequestParamsConfigIn>(config: Config) =>
  config.validate as RequestParamsConfigInValidate<Config>;

export type TransformedRequestParamsConfig<Config extends RequestParamsConfigIn> =
  RequestParamsConfig<
    RequestParamsConfigInValidator<Config>,
    CoercedTrue<RequestParamsConfigInValidate<Config>>
  >;
export const transformRequestParamConfig = <const CParams extends RequestParamsConfigIn>(
  config: CParams,
) =>
