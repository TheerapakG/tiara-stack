...
import { PermissionResolvable, PermissionsBitField, RoleResolvable } from "discord.js";
import { Data, Effect, Option, String, pipe } from "effect";
import { ClientService } from "./clientService";
import { CachedInteractionContext, InteractionContext, InteractionT } from "./interactionContext";
...
export class PermissionService extends Effect.Service<PermissionService>()("PermissionService", {
  effect: pipe(
    Effect.Do,
    bindObject({
      interaction: pipe(InteractionContext.interaction<InteractionT>().sync(), Effect.either),
    ...
    Effect.map(({ interaction, cachedInteraction, applicationOwner }) => ({
            ...
            pipe(
              ...
              bindObject({
                guildGuildId: pipe(
                  ...
                  Effect.flatMap(Effect.transposeMapOption((guildService) => guildService.getId())),
                ),
              }),
              Effect.map(
                ({ interactionUser, interactionGuildId, guildGuildId }) =>
                  (allowSameGuild &&
                    Option.getEquivalence(String.Equivalence)(interactionGuildId, guildGuildId)) ||
                ...
                Option.match({
                  onSome: (guildService) =>
                    pipe(
                      ...
                      Effect.map(
                        Option.match({
                          onSome: (member) => member.permissions.has(permissions),
                          onNone: () => false,
                        }),
                      ),
                    ),
                  onNone: () =>
                    pipe(
                      interaction,
                      Effect.map(
                        (interaction) => interaction.memberPermissions?.has(permissions) ?? false,
                      ),
                    ),
                }),
              ),
      ...
      privateCheckRoles: ({ roles, reason }: { roles: RoleResolvable[]; reason?: string }) =>
        pipe(
          Effect.fail(
            new PermissionError(reason ?? `You do not have necessary role to use this command`),
          ),
          Effect.unlessEffect(
            pipe(
              Effect.serviceOption(GuildService),
