...
export type GuildMemberKind<F extends BaseGuildMemberT> = HKT.Kind<
  F,
  never,
  never,
  never,
...
export class GuildMemberContext<M extends BaseGuildMemberT = GuildMemberT> {
  ...
  static make = <M extends BaseGuildMemberT = GuildMemberT>(
    member: GuildMemberKind<M>,
  ) => Context.make(this.guildMemberTag<M>(), member);

  static addRoles = wrap(
    (roleOrRoles: RoleResolvable | readonly RoleResolvable[]) =>
      pipe(
        GuildMemberContext.guildMember<GuildMemberT>().sync(),
        Effect.flatMap((member) =>
          DiscordError.wrapTryPromise(() => member.roles.add(roleOrRoles)),
        ),
  ...
  static setRoles = wrap((roles: readonly RoleResolvable[]) =>
    pipe(
      GuildMemberContext.guildMember<GuildMemberT>().sync(),
      Effect.flatMap((member) =>
        DiscordError.wrapTryPromise(() => member.roles.set(roles)),
      ),
