  ...
  effect: pipe(
    ...
    Effect.map(({ db, dbSubscriptionContext }) => ({
              ...
              Schema.decode(
                Result.ResultSchema({
                  optimistic: Schema.EitherFromSelf({
                    right: Schema.OptionFromNullishOr(DefaultTaggedClass(MessageSlot), undefined),
      ...
      upsertMessageSlotData: (
        messageId: string,
        data: Omit<MessageSlotInsert, "id" | "createdAt" | "updatedAt" | "deletedAt" | "messageId">,
      ) =>
        pipe(
          ...
          Effect.flatMap(
            Array.match({
              ...
              onEmpty: () => Effect.die(makeDBQueryError("Failed to upsert message slot data")),
            }),
          ),
