import { monitorGetByNameHandlerData } from "@/server/handler/data";
...
export const getMonitorByNameHandler = pipe(
  builders.empty(),
  builders.data(monitorGetByNameHandlerData),
  builders.handler(
    stripHandler(
      pipe(
        Effect.Do,
        Effect.tap(() => pipe(Event.someToken(), Effect.flatMap(AuthService.verify))),
        Effect.bind("parsed", () => Event.request.parsed(monitorGetByNameHandlerData)),
        ...
        Effect.map(({ parsed, layerOfGuildId }) =>
          pipe(
            ...
            Effect.flatMap((layerOfGuildId) =>
              pipe(
                Sheet.MonitorService.getByNames(
                  pipe(
                    parsed,
        ...
        Effect.map(Handler.Data.encodeResponseEffect(monitorGetByNameHandlerData)),
        Effect.map(
          Effect.withSpan("getMonitorByNameHandler", {
            captureStackTrace: true,
          }),
        ),
        Effect.withSpan("getMonitorByNameHandler subscription", {
          captureStackTrace: true,
        }),
      ),
    ),
