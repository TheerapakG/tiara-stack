import { monitorGetByIdHandlerData } from "@/server/handler/data";
...
export const getMonitorByIdHandler = pipe(
  builders.empty(),
  builders.data(monitorGetByIdHandlerData),
  builders.handler(
    stripHandler(
      pipe(
        Effect.Do,
        Effect.tap(() => pipe(Event.someToken(), Effect.flatMap(AuthService.verify))),
        Effect.bind("parsed", () => Event.request.parsed(monitorGetByIdHandlerData)),
        ...
        Effect.map(Handler.Data.encodeResponseEffect(monitorGetByIdHandlerData)),
        Effect.map(
          Effect.withSpan("getMonitorByIdHandler", {
            captureStackTrace: true,
          }),
        ),
        Effect.withSpan("getMonitorByIdHandler subscription", {
          captureStackTrace: true,
        }),
      ),
    ),
