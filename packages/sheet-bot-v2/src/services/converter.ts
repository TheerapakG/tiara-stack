...
export class ConverterService extends Effect.Service<ConverterService>()("ConverterService", {
  effect: pipe(
    Effect.all({ sheet: SheetService }),
    Effect.map(({ sheet }) => ({
      convertHourToHourWindow: Effect.fn("ConverterService.convertHourToHourWindow")(
        (guildId: string, hour: number) =>
          pipe(
            sheet.getEventConfig(guildId),
            Effect.map(
              (eventConfig) =>
                new HourWindow({
                  start: pipe(
      ...
      convertDateTimeToHour: Effect.fn("ConverterService.convertDateTimeToHour")(
        (guildId: string, dateTime: DateTime.DateTime) =>
          pipe(
            sheet.getEventConfig(guildId),
