...
const makeUnsetSubCommand = Effect.gen(function* () {
  ...
  return yield* CommandHelper.makeSubCommand(
    ...
    Effect.fn("channel.unset")(function* (command) {
      ...
      const config = yield* guildConfigService.upsertGuildChannelConfig(guildId, channelId, {
        ...(Option.getOrUndefined(running) ? { running: undefined } : {}),
        ...(Option.getOrUndefined(name) ? { name: undefined } : {}),
        ...(Option.getOrUndefined(role) ? { roleId: undefined } : {}),
        ...(Option.getOrUndefined(checkinChannel) ? { checkinChannelId: undefined } : {}),
      });
