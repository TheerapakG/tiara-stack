...
import { MembersCache, RolesCache } from "../discord/cache";
import { GuildMember } from "../utils";
...
const makeManualSubCommand = Effect.gen(function* () {
  ...
  return yield* CommandHelper.makeSubCommand(
    ...
    Effect.fn("kickout.manual")(function* (command) {
      ...
      yield* Effect.all({
        isOwnerOrInGuild: permissionService.checkInteractionUserApplicationOwner().pipe(
      ...
      if (minute >= 40) {
        yield* Effect.fail(new TimeError({ message: "Cannot kick out until next hour starts" }));
        return;
      }
