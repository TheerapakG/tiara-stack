...
export const smokeWorkflowTaskLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const enabled = yield* config.clusterSmokeWorkflowEnabled;
    if (!enabled) {
      return;
    }
    ...
    yield* Effect.gen(function* () {
      ...
      yield* Effect.logInfo("Ran sheet-cluster smoke workflow", result);
    }).pipe(Effect.withSpan("sheet-cluster.task.smokeWorkflow"), Effect.forkScoped);
  }),
);
